...
const STICKER_SECTIONS: { label: string; items: string[] }[] = [
  {
    label: 'Tinh yeu',
    items: [
      '❤️', '🩷', '🧡', '💛', '💚', '🩵', '💙', '💜', '🤍', '🖤',
...
export default function StickerOverlay({
  ...
  return (
    <div className="absolute inset-0 z-20" onClick={onContainerClick}>
      {/* ── Sticker items ─────────────────────────────────────────────────── */}
      ...
      {showPicker && (
        <div
          className="absolute bottom-14 left-3 z-40 w-64 bg-black/85 backdrop-blur-sm rounded-2xl shadow-2xl flex flex-col"
        ...
        >
          ...
          <div className="flex items-center justify-between px-3 pt-3 pb-1 shrink-0">
            ...
            <button
              ...
              aria-label="Dong picker"
              className="grid place-items-center w-6 h-6 rounded-full text-white/50 hover:text-white hover:bg-white/15 transition-colors"
            >
              <X size={12} />
            </button>
          </div>

          {/* Scrollable sticker grid, grouped by section */}
          <div className="overflow-y-auto overscroll-contain flex-1 px-2 pb-3">
            {STICKER_SECTIONS.map((section) => (
              <div key={section.label} className="mb-2">
                <p className="text-white/40 text-[10px] font-medium uppercase tracking-wide px-1 mb-1 pt-1">
                  {section.label}
                </p>
