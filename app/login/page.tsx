...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-pink-50 to-rose-50 relative overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute top-20 left-10 w-32 h-32 bg-rose-200 rounded-full mix-blend-multiply filter blur-xl opacity-40 animate-pulse" />
      <div className="absolute bottom-20 right-10 w-40 h-40 bg-pink-200 rounded-full mix-blend-multiply filter blur-xl opacity-40 animate-pulse" />
      <div className="absolute top-1/2 left-1/2 w-36 h-36 bg-red-100 rounded-full mix-blend-multiply filter blur-xl opacity-30 animate-pulse delay-700" />
          ...
          <div className={`flex gap-3 justify-center transition-transform duration-300 ${shake ? 'animate-shake' : ''}`}>
            {passcode.map((digit, index) => (
              <input
                ...
                disabled={loading}
                className={`w-14 h-14 text-center text-2xl font-bold rounded-xl border-2 transition-all duration-300 focus:outline-none cursor-pointer
